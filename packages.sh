...
if [[ "$JSON" ]];
then
    dotnet list ./Dosaic.sln package --format json
fi
...
then
    ...
    dotnet list ./Dosaic.sln package --format json | jq -r '[.projects[].frameworks[].topLevelPackages[] | {package: .id , version: .resolvedVersion }] | unique | .[] | "* [![Static Badge](https://img.shields.io/badge/"+(.package| sub("-";"--";"g"))+"-"+(.version | sub("-";"--";"g"))+"-blue)](https://www.nuget.org/packages/"+(.package)+")"'
fi

if [[ "$NAMES" ]];
then
    dotnet list ./Dosaic.sln package --format json | jq -r '[.projects[].frameworks[].topLevelPackages[] | {package: .id, version:.resolvedVersion }] | unique | .[].package'
fi

if [[ "$PACKAGES" ]];
then
    dotnet list ./Dosaic.sln package --format json | jq -r '[.projects[].frameworks[].topLevelPackages[] | {package: .id, version:.resolvedVersion }] | unique | .[] | "<PackageVersion Version="+"\""+(.version) +"\" Include="+"\""+(.package)+"\" />"'
