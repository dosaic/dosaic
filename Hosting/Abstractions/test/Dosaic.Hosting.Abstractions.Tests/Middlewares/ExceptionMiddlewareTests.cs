...
namespace Dosaic.Hosting.Abstractions.Tests.Middlewares
{
    public class ExceptionMiddlewareTests
    {
        ...
        [Test]
        public async Task MiddlewareHandlesValidationDosaicException()
        {
            var middleware = new ExceptionMiddleware(
                x => throw new ValidationDosaicException("failure", new List<FieldValidationError> { new("x", "1 > x") }),
        ...
        [Test]
        public async Task MiddlewareHandlesDosaicException()
        {
            var middleware = new ExceptionMiddleware(
                x => throw new DosaicException("test", 408),
                NullLogger<ExceptionMiddleware>.Instance,
                new GlobalStatusCodeOptions().Clear(),
                _dateTimeProvider);
            var context = GetContext();
