...
namespace Dosaic.DevTools.Seeding.Tests
{
    public class EfFakeDataSeederTests
    {
        ...
        [Test]
        public async Task SeedWithIgnoreSkipsEntityType()
        {
            await using var db = new TestDb();
            var config = EfFakeDataSeederConfig.For(db).WithIgnore<OrderLine>();
            var seeder = new EfFakeDataSeeder(config);
            await seeder.SeedAsync(CancellationToken.None);

            var orderLines = await db.Set<OrderLine>().ToListAsync();
            orderLines.Should().BeEmpty();

            // All other types should still be seeded
            var customers = await db.Set<Customer>().ToListAsync();
            customers.Should().NotBeEmpty();
        }

        [Test]
        public async Task SeedWithTotalCountRespectsLimit()
        {
            await using var db = new TestDb();
            var config = EfFakeDataSeederConfig.For(db).WithTotalCount<Customer>(3).WithIgnore<Order>().WithIgnore<OrderLine>();
            var seeder = new EfFakeDataSeeder(config);
            await seeder.SeedAsync(CancellationToken.None);
        ...
        [Test]
        public async Task SeedWithRelationCountOptions()
        {
            await using var db = new TestDb();
            var config = EfFakeDataSeederConfig.For(db)
                .WithTotalCount<Customer>(2)
