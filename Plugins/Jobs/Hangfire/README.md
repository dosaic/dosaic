...
Dosaic.Plugins.Jobs.Hangfire is a `plugin` that allows `Dosaic based services` to `use the hangfire package to manage and schedule (cron) jobs`.
...
**Feature management with the [dosaic unleash plugin](https://dosaic.gitbook.io/dosaic/plugins/management/unleash)**

This works in conjunction with the plugin Dosaic.Plugins.Management.Unleash.
...
The dosaic unleash plugin then should do the rest of the heavy lifting.
...
based on how often the dosaic unleash plugin refreshes its cache.
You can find more information in the documentation of the
[dosaic unleash plugin](https://dosaic.gitbook.io/dosaic/plugins/management/unleash).
