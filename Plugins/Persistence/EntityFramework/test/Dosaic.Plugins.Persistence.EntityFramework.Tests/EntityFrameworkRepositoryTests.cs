...
namespace Dosaic.Plugins.Persistence.EntityFramework.Tests
{
    public class EntityFrameworkRepositoryTests
    {
        ...
        public async Task UpdateAsyncCanReturnNotFound()
        {
            ...
            await repository.Invoking(async x => await x.UpdateAsync(entity)).Should().ThrowAsync<DosaicException>();
        }
        ...
        public async Task RemoveAsyncCanReturnNotFound()
        {
            await using var context = CreateContext();
            var repository = GetRepository(context);
            await repository.Invoking(async x => await x.RemoveAsync(Guid.NewGuid())).Should().ThrowAsync<DosaicException>();
        }
    }
}
