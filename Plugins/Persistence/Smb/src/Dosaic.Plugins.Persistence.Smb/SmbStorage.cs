...
namespace Dosaic.Plugins.Persistence.Smb
{
    ...
    {
        private readonly Counter<long> _writeCounter = Metrics.CreateCounter<long>("dosaic_persistence_smb_writes_total",
            "calls", "amount of total write calls");

        private readonly Counter<long> _readCounter = Metrics.CreateCounter<long>("dosaic_persistence_smb_reads_total",
            "calls", "amount of total read calls");

        private readonly Counter<long> _deleteCounter = Metrics.CreateCounter<long>("dosaic_persistence_smb_deletes_total",
        ...
        public async Task DeleteIfExists(string filePath, CancellationToken cancellationToken = default)
        {
            try
            {
                ...
                if (node is not null)
                {
                    ...
                    if (!isDeleted)
                        throw new DosaicException($"Could not delete SMB Node: '{filePath}'");
